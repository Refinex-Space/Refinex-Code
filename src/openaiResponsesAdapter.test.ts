...
describe("openai responses adapter", () => {
  ...
  test("normalizes strict built-in tool schemas for OpenAI function calling", async () => {
    ...
    expect(
      bashTool.parameters.properties.dangerouslyDisableSandbox.type,
  ...
  test("maps built-in web_search_20250305 schema to codex-compatible web_search tool type", async () => {
    let capturedInit: RequestInit | undefined;
    globalThis.fetch = mock(
      async (_input: RequestInfo | URL, init?: RequestInit) => {
        capturedInit = init;
        return new Response(
          JSON.stringify({
            id: "resp_web_search_tool",
            model: "gpt-5.4",
            output: [
              {
                id: "msg_web_search_tool",
                type: "message",
                role: "assistant",
                content: [
                  {
    ...
    await queryModelWithoutStreaming({
      messages: [createUserMessage({ content: "search docs" })],
      systemPrompt: asSystemPrompt(["system prompt"]),
      thinkingConfig: { type: "disabled" },
      tools: [],
      signal: new AbortController().signal,
    ...
    const webSearchTool = body.tools.find(
      (entry: { type?: string }) => entry.type === "web_search",
    );
  ...
  test("maps web_search_call with annotation-only citations into web_search_tool_result", async () => {
    globalThis.fetch = mock(async () => {
      return new Response(
        JSON.stringify({
          id: "resp_web_search_annotations",
          model: "gpt-5.4",
          output: [
            {
              id: "msg_web_search_annotations",
              type: "message",
              role: "assistant",
              content: [
                {
                  type: "output_text",
                  text: "Harness Engineering 的核心思想包括 agent-first 控制面。",
                  annotations: [
                    {
                      type: "url_citation",
                      title: "Harness Engineering Guide 2026",
                      url: "https://example.com/harness-engineering-2026",
                    },
                  ],
                },
              ],
    ...
    const result = await queryModelWithoutStreaming({
      messages: [createUserMessage({ content: "search harness engineering" })],
      systemPrompt: asSystemPrompt(["system prompt"]),
      thinkingConfig: { type: "disabled" },
      tools: [],
      signal: new AbortController().signal,
    ...
    const content = Array.isArray(result.message.content)
      ...
      : [];
    const webSearchResult = content.find(
      (block) =>
        typeof block === "object" &&
        block !== null &&
        "type" in block &&
        block.type === "web_search_tool_result",
    ) as { content?: Array<{ title?: string; url?: string }> } | undefined;

    expect(webSearchResult).toBeDefined();
    expect(webSearchResult?.content).toEqual([
      {
        title: "Harness Engineering Guide 2026",
        url: "https://example.com/harness-engineering-2026",
      },
