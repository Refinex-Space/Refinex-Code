...
function normalizeSchemaForOpenAIStrict(
  ...
  if (isRecord(normalized.properties)) {
    ...
    normalized.properties = Object.fromEntries(
      Object.entries(normalized.properties).map(([propertyName, propertySchema]) => [
        propertyName,
...
function mapExtraToolSchema(extraToolSchema: Record<string, unknown>): OpenAITool | null {
  const type = typeof extraToolSchema.type === 'string' ? extraToolSchema.type : ''
...
function convertMessagesToResponsesInputItems(messages: Message[], tools: Tools): OpenAIInputItem[] {
...
function mapResponseUsage(response: OpenAIResponse): AssistantMessage['message']['usage'] {
...
function mapResponseOutputToBlocks(response: OpenAIResponse): BetaContentBlock[] {
  ...
  for (const item of response.output ?? []) {
    ...
    if (item.type === 'function_call') {
      ...
      blocks.push({
        ...
        input:
          (safeParseJSON(functionCall.arguments, false) as Record<string, unknown>) ??
          {},
      } as BetaContentBlock)
      continue
    }
...
function mapResponseToAssistantMessage(response: OpenAIResponse): AssistantMessage {
...
const queryModelWithoutStreaming: QueryModelWithoutStreamingSignature = async ({
  ...
  try {
    ...
    if (
      ...
      return createMissingAuthMessage(getConfiguredModelProviderInfo().resolvedId)
    }
    return createOpenAIErrorMessage(error)
  }
}

const queryModelWithStreaming: QueryModelWithStreamingSignature = async function* ({
  messages,
  systemPrompt,
  tools,
  signal,
  ...
  try {
    ...
    while (true) {
      ...
      for (const frame of frames) {
        ...
        const payload = safeParseJSON(frame.data, false) as
          | { type?: string; delta?: string; response?: OpenAIResponse }
          | null
        if (!payload) {
          continue
        }
    ...
    yield {
      ...
      event: {
        ...
        delta: {
          stop_reason: assistantMessage.message?.stop_reason ?? 'stop_sequence',
        },
      },
    }
    yield {
