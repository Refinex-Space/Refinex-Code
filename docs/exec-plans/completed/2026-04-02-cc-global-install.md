...
> Summary: Added a global `ccc` launcher path and repository-aware startup scanning for the restored CLI.
> Duration: Historical record migrated from the legacy `superpowers` workflow on 2026-04-05.
> Key learnings: Keep launcher wrappers import-only and resolve repository paths from the entry file location, not from `process.cwd()`.

> Archived from `docs/superpowers/plans/2026-04-02-cc-global-install.md` during Harness migration. The historical implementation plan is preserved below with minimal changes.
